...
const Navbar: React.FC = () => {
  ...
  const handleNavigation = (id: string) => {
    ...
    if (location.pathname !== '/') {
      // If we are on Admin page or other pages, navigate to home first
      navigate('/');
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center">
          <div className="flex items-center">
            <button onClick={() => handleNavigation('home')} className="flex items-center gap-3 group focus:outline-none">
          ...
          <div className="hidden md:flex space-x-6 items-center">
            {navLinks.map((link) => (
              <button
                ...
                className="text-xs lg:text-sm font-medium text-slate-300 hover:text-brand-500 transition-colors tracking-widest uppercase font-sans focus:outline-none"
              >
                {link.name}
              </button>
            ))}
            ...
            <button 
              ...
              className="px-6 py-2 bg-brand-600 hover:bg-brand-500 text-white text-xs font-bold tracking-widest uppercase rounded-sm transition-all shadow-lg shadow-brand-900/50 hover:shadow-brand-500/30 hover:-translate-y-0.5"
            >
              {t.book}
            </button>
          </div>
          ...
          <div className="px-6 pt-4 pb-8 space-y-2">
            {navLinks.map((link) => (
              <button
                key={link.name}
                onClick={() => handleNavigation(link.id)}
                className="block w-full text-left px-4 py-4 text-lg font-serif font-medium text-slate-300 hover:text-white hover:bg-slate-900 rounded-lg border-b border-slate-800/50"
              >
                {link.name}
              </button>
            ))}
